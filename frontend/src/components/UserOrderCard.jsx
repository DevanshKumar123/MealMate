    ...
    return (
        <div className='bg-white rounded-lg shadow p-4 space-y-4'>
            <div className='flex justify-between border-b pb-2'>
                <div>
                    <p className='font-semibold'>
                        order #{data && data._id ? String(data._id).slice(-6) : 'N/A'}
                    </p>
                    <p className='text-sm text-gray-500'>
                        Date: {data && data.createdAt ? formatDate(data.createdAt) : 'N/A'}
                    </p>
                </div>
                <div>
                    <div className='text-right'>
                        {data && data.paymentMethod === "cod"
                            ...
                            : <p className='text-sm text-gray-500 font-semibold'>Payment : {data && data.payment ? "true" : "false"}</p>}
                        <p className='font-medium text-blue-600'>
                            {Array.isArray(data?.shopOrders) && data.shopOrders[0]?.status ? data.shopOrders[0].status : ''}
                        </p>
                    </div>
                </div>
            </div>
            ...
            {Array.isArray(data?.shopOrders) && data.shopOrders.length > 0 ? (
                data.shopOrders
                    .filter(shopOrder => shopOrder && shopOrder.shop && shopOrder.shop.name && Array.isArray(shopOrder.shopOrderItems))
                    .map((shopOrder, index) => (
                        <div className='border rounded-lg p-3 bg-[#fffaf7] space-y-3' key={index}>
                            <p>{shopOrder.shop.name}</p>
                            <div className='flex space-x-4 overflow-x-auto pb-2'>
                                {shopOrder.shopOrderItems
                                    .filter(item => item && item.item && item.item._id)
                                    .map((item, idx) => (
            ...
            <div className='flex justify-between items-center border-t pt-2'>
                <p className='font-semibold'>Total: ₹{data && data.totalAmount ? data.totalAmount : 0}</p>
                <button
                    className='bg-[#00fb7d] hover:bg-[#00d965] text-black px-4 py-2 rounded-lg text-sm'
                    onClick={() => data && data._id && navigate(`/track-order/${data._id}`)}
                >Track Order</button>
            </div>
        </div>
    )
