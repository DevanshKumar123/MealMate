...
function DeliveryBoy() {
  ...
  const getAssignments = async () => {
    try {
      ...
      console.error("Get assignments error:", error.response?.data?.message || error.message);
  ...
  const getCurrentOrder = async () => {
    try {
      ...
      console.error("Get current order error:", error.response?.data?.message || error.message);
      setCurrentOrder(null);
    }
  };
  ...
  const handleTodayDeliveries = async () => {
    try {
      ...
      console.error("Get today deliveries error:", error.response?.data?.message || error.message);
      setTodayDeliveries([]);
    }
  };
      ...
      <div className="w-full max-w-[800px] flex flex-col gap-5 items-center">
        <div className="bg-white rounded-2xl shadow-md p-5 flex flex-col justify-start items-center w-[90%] border border-orange-100 text-center gap-2">
          ...
          <p className="text-[#ff4d2d]">
            ...
            {deliveryBoyLocation?.lat || "NA"} ,{" "}
            <span className="font-semibold">Longitude:</span>{" "}
            {deliveryBoyLocation?.lon || "NA"} ,{" "}
          </p>
        </div>
